...
This document provides complete endpoint specifications, HTTP methods, headers, parameters, and sample request/response payloads for the marketplace platform backend APIs.

---

## 1. Shopify OAuth Auth Initiation (`GET /api/shopify/auth`)
...
## 2. Shopify OAuth Callback (`GET /api/shopify/callback`)
...
## 3. Catalog Sync Endpoint (`POST /api/shopify/sync`)
...
## 4. Webhook Ingestion Listener (`POST /api/webhooks/shopify`)
...
- **Headers**:
  ...
  - `x-shopify-topic` (required, string): Topic name (e.g. `products/update`).
...
```json
{
  ...
  "location_id": 491028,
  "available": 45,
  "updated_at": "2026-08-08T11:28:00.000Z"
}
```
...
```json
{
  ...
  "entityId": null,
  "timestamp": "2026-08-08T11:28:01.000Z",
  "status": "SYNC_LOGGED"
}
```
...
- `500 Internal Server Error`: Internal processing exception.
