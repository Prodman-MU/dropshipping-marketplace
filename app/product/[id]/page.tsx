  ...
  return (
    <div className="min-h-screen bg-[#F4F4F0] flex flex-col font-sans">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        <div className="bg-white border-4 border-[#111111] shadow-[10px_10px_0px_#111111] p-4 sm:p-6 lg:p-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                <div className="flex items-center gap-3 overflow-x-auto pb-1">
                  {images.map((img, idx) => (
                    <button
                      ...
                      className={`w-16 h-16 border-2 border-[#111111] shrink-0 transition-all ${
            ...
            {/* Right Column: Title, Description, Price, Actions & Vendor Box */}
            <div className="lg:col-span-6 space-y-6">
              ...
              <div className="space-y-2">
                ...
                <h1 className="text-2xl sm:text-3xl font-black text-[#111111] font-display uppercase tracking-tight leading-tight">
                  {slot.title}
                </h1>
              </div>
              ...
              <div className="space-y-1.5 font-mono text-xs">
                <span className="text-[10px] font-bold text-[#2B2D42] uppercase block">Product Description & Specification:</span>
                <p className="text-zinc-800 leading-relaxed font-sans text-sm sm:text-base font-semibold bg-[#F4F4F0] p-4 border-2 border-[#111111] shadow-[2px_2px_0px_#111111]">
                  {slot.description}
                </p>
              </div>

              {/* 3. Price & Stock Status Box */}
              <div className="bg-[#FFB703] border-3 border-[#111111] p-4 shadow-[4px_4px_0px_#111111] space-y-2">
                <div className="flex items-baseline justify-between gap-3 flex-wrap">
                  <div className="flex items-baseline gap-3">
                    ...
                    {slot.compareAtPrice && slot.compareAtPrice > slot.price && (
                      <span className="text-sm font-mono text-zinc-700 line-through font-bold">
                  ...
                  {((!slot.isUnknownQuantity && slot.inventoryQuantity <= 0) || slot.status === "SOLD" || !currentVariant.availableForSale) && (
                    <span className="px-2.5 py-1 text-xs font-mono font-black bg-[#D62828] text-white border-2 border-[#111111] shadow-[2px_2px_0px_#111111] uppercase">
                      OUT OF STOCK
                    </span>
                  )}
                </div>
                <span className="text-[11px] font-mono font-bold text-[#111111] block">
                  Wholesale Dropship Unit Cost (Taxes Included)
                </span>
              </div>

              {/* 4. Variant Selector (If Multiple Variants Exist) */}
              {variants.length > 1 && (
                <div className="space-y-2">
                  <label className="text-xs font-mono font-bold text-[#111111] uppercase tracking-wider block">
                    Select Product Variant ({variants.length} Available):
                  </label>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    {variants.map((v, idx) => (
                      ...
                      >
                        <span className="font-black block line-clamp-1">{v.title}</span>
              ...
              {/* 5. Action CTAs for Buying & Inquiries */}
              <div className="space-y-3 pt-1">
                ...
                <a
                  href={`https://wa.me/${slot.merchant.whatsappNumber || "919876543210"}?text=${encodeURIComponent(
                    `Hi ${slot.merchant.name}! I want to dropship your product "${slot.title}" (SKU: ${slot.sku}). Price: ${formatCurrency(slot.price, slot.currencyCode || "INR")}. Please share B2B vendor terms.`
                  )}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-full py-3.5 px-5 bg-[#25D366] hover:bg-[#128C7E] text-white border-3 border-[#111111] text-xs sm:text-sm font-mono font-black flex items-center justify-center gap-2.5 shadow-[4px_4px_0px_#111111] hover:shadow-[2px_2px_0px_#111111] transition-all uppercase tracking-wider"
                >
                ...
                {slot.productUrl ? (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-full py-3.5 px-5 bg-[#111111] hover:bg-[#D62828] text-white border-3 border-[#111111] text-xs sm:text-sm font-mono font-black flex items-center justify-center gap-2 shadow-[4px_4px_0px_#FFB703] hover:shadow-[2px_2px_0px_#FFB703] transition-all uppercase tracking-wider"
                  >
                    <ShoppingBag className="w-4 h-4 text-[#FFB703]" />
                    <span>DIRECT CHECKOUT ON SHOPIFY STORE</span>
                  ...
                  >
                    ...
                    <span>VISIT VENDOR SHOPIFY STOREFRONT</span>
                    <ExternalLink className="w-4 h-4" />
                  </a>
                )}
                ...
                <button
                  ...
                  className="w-full py-2.5 px-3 bg-white hover:bg-[#F4F4F0] text-[#111111] border-2 border-[#111111] font-mono text-xs font-bold flex items-center justify-center gap-2 shadow-[2px_2px_0px_#111111] transition-all"
                >
              ...
              {/* 6. Separate Vendor Information Box */}
              <div className="bg-white border-3 border-[#111111] p-4 shadow-[4px_4px_0px_#111111] space-y-3 font-mono">
                <div className="flex items-center justify-between border-b-2 border-[#111111] pb-2">
                  <span className="text-[11px] font-black text-[#111111] uppercase tracking-wider flex items-center gap-1.5">
                    <Store className="w-3.5 h-3.5 text-[#D62828]" />
                    <span>Fulfilling Merchant Vendor Information</span>
                  </span>
                  <span className="inline-flex items-center gap-1 text-emerald-700 font-bold bg-emerald-100 border border-emerald-500 px-2 py-0.5 text-[10px]">
                ...
                <div className="flex items-center gap-3">
                  {slot.merchant.storeLogo ? (
                    <img
                      ...
                      className="w-12 h-12 border-2 border-[#111111] object-cover bg-white shrink-0 shadow-[2px_2px_0px_#111111]"
                    />
                  ) : (
                    <div className="w-12 h-12 bg-[#FFB703] border-2 border-[#111111] flex items-center justify-center font-black text-sm shrink-0 shadow-[2px_2px_0px_#111111]">
                      {slot.merchant.name.slice(0, 2).toUpperCase()}
                    </div>
                  )}
                  <div className="space-y-0.5">
                    <h4 className="text-base font-black text-[#111111] font-display line-clamp-1">
                      {slot.merchant.name}
                    </h4>
                    <a
                      href={`https://${slot.merchant.myshopifyDomain}`}
      ...
      {/* Footer Copyright */}
      <footer className="border-t-4 border-[#111111] bg-white mt-12 py-6 text-center font-mono text-xs font-bold text-[#111111]">
